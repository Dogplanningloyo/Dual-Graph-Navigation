...
class RRT_Planner:
    def __init__(self, map_info,obstacles, max_iter=100, step_size=10, goal_sample_rate=0.05):
        self.start = Node(0, 0)
        self.goal = Node(0, 0)
    ...
    def is_collision(self, node):
        ...
        if node.x-self.min_x >= self.obstacles.shape[1] or node.y >= self.obstacles.shape[0]:
            print("Off the map")
            return True
    ...
    def plan(self,start, goal,show=False):
...
if __name__ == '__main__':
    ...
    rrt_1 = time.time()
    rrt = RRT_Planner(map_info, obstacles, max_iter, step_size, goal_sample_rate)
    path,judge = rrt.plan(start, goal,show=False)
    rrt_2 = time.time()
